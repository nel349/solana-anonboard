...
Three things make it original.

- **It pairs two chains that have never been paired here.** EffectStream already has Midnight paired with EVM, Bitcoin, and Cardano, but not Solana. This would be the first.
- **The privacy proof gates the action, and the rollup enforces it.** In the existing cross-chain templates, the two chains are matched up afterwards in the node's database, so nothing stops an action coming from an account that never qualified. Here it is the reverse: a post is only counted if the author first proved membership on Midnight. No proof, no post. ([`zk-cardano`](https://github.com/effectstream/effectstream/tree/v-next/templates/zk-cardano) flags this same gap in its own README: its eligibility is not checked in the circuit.)
- **It goes past bboard.** bboard hides who posted, but any wallet can post and it holds only one message at a time. This adds what bboard has no concept of: membership, and exactly one badge per member.

### Why it is feasible

The composition is the framework's normal shape, not new engine work. EffectStream is already built to read several chains side by side under one shared timeline that orders their blocks together. `evm-midnight-v2` runs EVM and Midnight that way, and `solana-starter` runs Solana the same way, so combining the two is wiring, not new framework work. The privacy path is already shown by `zswap-da`, which proves in the browser and calls `balanceUnsealedTransaction` with `payFees: false`. The wallet seals a locally-proven transaction without spending its own Dust and the batcher pays, so the batcher never sees the private inputs.
...
To be clear on the chains: this dApp uses Solana and Midnight only, no EVM. I would reuse the Solana leg from `solana-starter`, the Midnight-side wiring from `evm-midnight-v2` (its Midnight integration only, not its EVM contract), the in-browser proving path from `zswap-da`, and the identity pattern from `example-bboard`, which is Midnight's own. I am not claiming any ZK novelty. What is new is the roster gate, the one-badge-per-person nullifier, and the cross-chain arbiter. I could not find an existing Solana and Midnight template, so this would also be the first pairing of those two chains.
...
I want to build a different version of the Midnight bulletin board. [`example-bboard`](https://github.com/midnightntwrk/example-bboard) hides *who posted*, but it cannot answer whether someone is **allowed** to post at all, because it has no idea of membership and any wallet can post. My version proves membership once on Midnight with a Compact circuit that checks a roster and burns a nullifier, so each member gets exactly one anonymous badge, and that badge is just a fresh Solana keypair. Posting then happens on Solana, so Midnight only holds the *right to speak* while the speech lives on the chain built for volume. The EffectStream state machine joins the chains: it reads the badge set from Midnight's ledger, reads posts from the Solana program log, and **counts a post only if its signer holds a badge**. Every post is then provably from a real member, and no post can be traced to a person, not even by whoever runs the servers, because the nullifier map is deliberately not exported from the contract. This is what makes it original rather than a reskin: in the existing cross-chain templates the two chains are matched up afterwards in the node's database, so an account that never qualified can still act, whereas here a post is only counted if the author first proved membership on Midnight (`zk-cardano` flags this same gap in its own README). On feasibility, the composition is the framework's normal shape, since `evm-midnight-v2` and `solana-starter` both already read their chains side by side under one shared timeline the framework provides, and the privacy path is already shown by `zswap-da`, which proves in the browser and calls `balanceUnsealedTransaction` with `payFees: false` so the batcher pays without seeing the private inputs. This dApp uses Solana and Midnight only, no EVM. I reuse the Solana leg from `solana-starter`, the Midnight-side wiring from `evm-midnight-v2` (its Midnight integration only, not its EVM contract), the browser proving path from `zswap-da`, and the identity pattern from `example-bboard`; the roster gate, the one-badge-per-person nullifier, and the cross-chain arbiter are new. I could not find an existing Solana and Midnight template, so this would also be **the first pairing of those two chains**.
