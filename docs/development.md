...
- **A browser wallet won't connect on `undeployed` (or a preprod Join fails).** Lace does not support `undeployed` at all ([lace#2254](https://github.com/input-output-hk/lace/issues/2254)), 1AM breaks when you switch between `undeployed` and a hosted net (upstream, fix pending), and on **preprod** a browser wallet's dust sync lags the large chain so the Join is rejected. For a browser Join in either case, connect the **mn CLI wallet** ([Using the mn CLI wallet](mn-wallet.md)); or use the headless flow (`bun run scripts/demo.ts` / `scripts/blind-join.ts`). On a hosted net, a stale "Network ID mismatch" clears by disconnecting and reconnecting (or reopening the tab).
